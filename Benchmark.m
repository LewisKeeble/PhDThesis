...
function generate_dataset_v3_3_HPC_Benchmark(burst_run)
%burst_run=4;
    tic
    %% Reaction
    num_of_mol = 2; % # of initial molecules
    ...
    %burst_array = 5*[1.8e13,1.8e14,1.8e15];
    ...
    time_step_array = [0.01,0.05,0.1,0.5,1];
    time_step = time_step_array(burst_run); % in seconds
    ...
    sensor_xsize = chip.N_x*(2*chip.electrodeisfet_separation+chip.isfet_width) + chip.N_electrodes*chip.electrode_width + chip.wall_separation_xpos + chip.wall_separation_xneg;
    sensor_ysize = chip.sensor_startdisplacement + chip.N_y*(chip.isfet_length) + (chip.N_y-1)*(chip.isfetisfet_separation) + chip.sensor_enddisplacement + chip.wall_separation_ypos + chip.wall_separation_yneg;
    
    reaction_xsize = 120e-6;
    reaction_ysize = 120e-6;
    solution_height = 100e-6;
    ...
    clusters.centre_x=[50e-6,50e-6];
    clusters.centre_y=[40e-6,80e-6];
    clusters.centre_z=[50e-6,50e-6];
    sigmoid_start=[1,11];
    t_start = 1;
    ...
    for i = t_start:length(t)-1
        ...
        for j = 1:num_of_active_sig
            ...
            numtorelease = round(burst*(volume_of_sphere(clusters.radius(j))  - volume_of_sphere(clusters.radius_prev(j)))/(speed_of_front*burst_factor));
            ...
            protons_released(i) = protons_released(i) + size(additions, 2);
    ...
    %         disp(protons_final)
            ...
            insensor(i)=size(protons_final,2);
            
            for idx = 1:size(protons_final,2)

                % Add proton to sensor
                sensor_nH((column_nearest(idx) - 1)*chip.N_y + row_nearest(idx), protons_final(4,idx)) = sensor_nH((column_nearest(idx) - 1)*chip.N_y + row_nearest(idx), protons_final(4,idx)) + 1;
            end
    ...
    runtime = toc
    output = cumsum(protons_released);
    disp(max(output))
    save(sprintf('SensornH_Benchmark_BurstFactor1_TimeStep%d.txt',time_step),'sensor_nH','-ASCII','-append')
    save(sprintf('ProtonsReleased_Benchmark_BurstFactor1_Burst%d.txt',time_step),'protons_released','-ASCII','-append')
