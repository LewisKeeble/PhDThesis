...
for i = t_start:length(t)-1
    ...
    if size(protons, 2) > 0
        ...
        protons_final(:, (protons_final(4,:)+i>length(t))) = [];
        ...
        for j = 1:size(protons_final,2)
            ...
            sensor_nH_reflecting((column_nearest(j) - 1)*chip.N_y + row_nearest(j), i + protons_final(4,j)) = sensor_nH_reflecting((column_nearest(j) - 1)*chip.N_y + row_nearest(j), i + protons_final(4,j)) + 1;
        end
        ...
        for j = 1:size(protons_final,2)
            ...
            % Add proton to sensor
            sensor_nH_absorbing((column_nearest(j) - 1)*chip.N_y + row_nearest(j), i + protons_final(4,j)) = sensor_nH_absorbing((column_nearest(j) - 1)*chip.N_y + row_nearest(j), i + protons_final(4,j)) + 1;
        end
...
save('SensornH_Kenny1e3_BurstFactor1e2_Reflecting_2.txt','sensor_nH_reflecting','-ASCII','-append')
save('SensornH_Kenny1e3_BurstFactor1e2_Absorbing_2.txt','sensor_nH_absorbing','-ASCII','-append')
save('ProtonsReleased_Kenny1e3_BurstFactor1e2_2.txt','protons_released','-ASCII','-append')
