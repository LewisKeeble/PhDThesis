...
function generate_dataset_v3_3_HPC_Kenny_NCopyVariation_1e3(run)
    ...
    if t_start < 0
       sigmoid_start = sigmoid_start + abs(t_start) + 1; 
       t = 0:time_step:total_time*60+t_start; % in seconds
       t_start = 1;
    end
    ...
    save(sprintf('SensornH_Kenny_BurstFactor1e2_Run%d.txt',run),'sensor_nH','-ASCII','-append')
    save(sprintf('ProtonsReleased_Kenny_BurstFactor1e2_Run%d.txt',run),'protons_released','-ASCII','-append')
end
